import { ship_iddle_2 } from "../../animations/iddleShip2";
...
export default class Enemy extends Phaser.GameObjects.Sprite {
  ...
  private init_sprite = (): void => {
    this.anims.create(ship_iddle_2); 
    this.play("spaceship_2");
    this.setAngle(180);
  }
  ...
  private init_body = (): void => {
    ...
    this.body.setOffset(16 / 2 + 4, 16 / 2)
  ...
  public shoot = (bullets: Phaser.GameObjects.Group) => {
    if (this.scene.time.now > this.last_shot) {
      const bullet = new Bullet1(this.scene, this.x - 1, this.y, 1);
      bullet.pattern_straight(100);
      bullets.add(bullet);
